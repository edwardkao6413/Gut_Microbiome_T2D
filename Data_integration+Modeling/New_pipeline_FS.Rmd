...
# val = qin_data
df = bind_rows(hmp_data, yuj_data, sank_data)
...
``` {r}
# ab.mint.splsda = tune(method = "mint.splsda", X = X, Y = Y, study = study, ncomp = 1, test.keepX = seq(10, ncol(X), 1), dist = "centroids.dist")  # 15
ab.mint.splsda = tune(method = "mint.splsda", X = X, Y = Y, study = study, ncomp = 2, test.keepX = seq(10, ncol(X), 1), already.tested.X = 15, dist = "centroids.dist")
plot(ab.mint.splsda) # 15 in comp1, # 71 in comp2 (error rate=0.399)
```

```{r}
ab.mint.splsda$error.rate
```
...
## derive taxa
```{r}
mint.splsda.model = mint.splsda(X = X, Y = Y, study = study, keepX = c(15, 71))
...
write.csv(loading_df, "C:\\Users\\edwar\\Desktop\\Melbourne\\research_project\\loadings_training_FS.csv", row.names = T)
```
...
```{r}
fs_overlap_prjeb = data.frame(taxa = intersect(ttl_taxa, colnames(df)))
write.csv(fs_overlap_prjeb, "C:\\Users\\edwar\\Desktop\\Melbourne\\research_project\\fs_prjeb_overl.csv", row.names = F)

# fs_overlap_qinj = data.frame(taxa = intersect(ttl_taxa, colnames(df)))
# write.csv(fs_overlap_qinj, "C:\\Users\\edwar\\Desktop\\Melbourne\\research_project\\fs_qinj_overl.csv", row.names = F)
...
## further working (see the modelling performance of overlapped derived taxa between 2 testing set)
```{r}
